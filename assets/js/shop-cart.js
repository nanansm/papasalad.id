...
export function addToCart({id,name,price,qty=1}){
  const c=readCart(); c.items[id]=(c.items[id]||0)+qty; c.catalog[id]={name,price:Number(price)||0}; writeCart(c); updateBadge();
}
...
// Attach listeners for product grids (delegation)
...
export function buildCartUI(rootSel='#cartBox'){
  ...
  ids.forEach(id=>{
    const q=c.items[id]; const meta=c.catalog[id]||{name:id,price:0}; const line=q*meta.price; sub+=line;
...
export async function checkoutWA({waNumber, sheetUrl, methodSel='#method', nameSel='#custName', phoneSel='#custPhone', notesSel='#notes'}){
  ...
  const name = (document.querySelector(nameSel)?.value||'').trim() || '-';
  const phone= (document.querySelector(phoneSel)?.value||'').trim() || '-';
  const notes= (document.querySelector(notesSel)?.value||'').trim() || '-';
  let rows=[]; let sub=0;
  ids.forEach(id=>{ const q=c.items[id], meta=c.catalog[id]||{name:id,price:0}; const line=q*meta.price; sub+=line;
    rows.push(`• ${meta.name} x${q} @ ${idr(meta.price)} = ${idr(line)}`); });

  // Optional: record to Google Sheet via Apps Script Web App
  if(sheetUrl){
    try{
      await fetch(sheetUrl, {
        method:'POST',
        mode:'no-cors', // Web App should set CORS if you want response
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ 
          ts: new Date().toISOString(),
          name, phone, notes, method,
          items: ids.map(id=>({ id, name:c.catalog[id]?.name||id, qty:c.items[id], price:c.catalog[id]?.price||0 })),
          subtotal: sub
        })
      });
